...
import ScheduledDelivery from '../models/ScheduledDelivery';
...
export const getHomeDashboard = async (req: Request, res: Response) => {
  try {
    ...
    let banners = await Campaign.find({ status: 'Active' }).populate('ownerId', 'name');
...
export const getPersonalizationDetails = async (req: Request, res: Response) => {
  try {
    let hasPets = true;
    let hasKids = true;
      ...
      try {
        ...
        if (decoded && decoded.id) {
          ...
          if (user) {
            ...
            if (user.hasPets !== undefined) hasPets = user.hasPets;
            if (user.hasKids !== undefined) hasKids = user.hasKids;
          }
        }
      } catch (err) {
    ...
    if (userIdStr && mongoose.Types.ObjectId.isValid(userIdStr)) {
      ...
      try {
        ...
        if (todayServices.length > 0) {
          const latestService = todayServices[todayServices.length - 1] as any;
    ...
    try {
      ...
      const activeCampaigns = await Campaign.find({ status: 'Active' }).populate('ownerId', 'name');
    ...
    try {
      if (userIdStr && mongoose.Types.ObjectId.isValid(userIdStr)) {
        ...
        const wishlist = await mongoose.model('Wishlist').findOne({ userId: userObjectId });
        
        const prodIds = new Set<string>();
        if (cart && cart.items) {
          cart.items.forEach((item: any) => {
            if (item.productId) prodIds.add(item.productId.toString());
          });
        }
        if (wishlist && wishlist.products) {
          wishlist.products.forEach((id: any) => {
