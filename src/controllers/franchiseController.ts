...
import { assignTerritoryAndMapFranchises } from './adminController';
...
export const autoAssignTerritoriesToFranchise = async (franchiseId: string) => {
  const franchise = await Franchise.findById(franchiseId);
  if (!franchise) throw new Error("Franchise not found");

  const filter: any = { state: franchise.state };
...
export const createFranchise = async (
  ...
  try {
    ...
    const existing = await Franchise.findOne({ userId: req.user.id });
    ...
    const assignedTerritoryIds = await autoAssignTerritoriesToFranchise(
      saved._id.toString()
    );

    const updatedFranchise = await Franchise.findById(saved._id)
      .populate(
        "assignedTerritories",
        "level name state district mandal pincode"
      )
      .populate(
        "parentFranchiseId",
        "businessName ownerName franchiseCode franchiseLevel"
      );
...
export const getFranchiseProfile = async (
  ...
  try {
    ...
    const franchise = await Franchise.findOne({ userId: req.user.id })
      .populate(
        "assignedTerritories",
        "level name state district mandal pincode parentId franchiseId status density targetCoverage"
      )
      .populate(
        "parentFranchiseId",
        "businessName ownerName franchiseCode franchiseLevel state district mandal"
      )
...
export const updateFranchiseProfile = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
    ...
    allowedFields.forEach(field => {
      if (updates[field] !== undefined) {
        if (field === 'bankDetails') {
          ...
          (franchise as any)[field] = updates[field];
        }
      }
    });
...
export const getFranchiseTeam = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
    ...
    let subFranchises: any[] = [];
    let entrepreneurs: any[] = [];
    let vendors: any[] = [];
    ...
    if (franchiseLevel === 'state') {
      ...
      entrepreneurs = await Entrepreneur.find({ state, district, mandal });
      const mappings = await TerritoryMapping.find({ mandalFranchiseId: franchise._id });
      const vendorIds = mappings.filter(m => m.businessType === 'vendor').map(m => m.businessId);
      const spIds = mappings.filter(m => m.businessType === 'service_provider').map(m => m.businessId);
    ...
    const populatedSubs = await Promise.all(subFranchises.map(async (sf) => {
      const sfSettlements = await CommissionSettlement.find({ recipientId: sf.userId }).populate('orderId');
      const commissionEarned = sfSettlements
        .filter(s => s.status === 'released')
        .reduce((sum, s) => sum + s.amount, 0);
      const revenue = sfSettlements.reduce((sum, s) => sum + ((s.orderId as any)?.totalAmount || 0), 0);
    ...
    const populatedVendors = await Promise.all(vendors.map(async (vObj) => {
      const v = vObj as any;
      const orders = await Order.find({ sellerId: v.userId });
    ...
    // MLM metrics
    const directReferrals = await mongoose.model("Referral").countDocuments({ referrerUserId: franchise.userId });
...
export const getFranchiseTerritories = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
    ...
    const territories = await TerritoryMapping.find(query);
...
export const getFranchisePerformance = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
    ...
    let query: any = {};
    if (franchise.franchiseLevel === 'state') query = { stateFranchiseId: franchise._id };
    else if (franchise.franchiseLevel === 'district') query = { districtFranchiseId: franchise._id };
    else query = { mandalFranchiseId: franchise._id };

    const mappingsCount = await TerritoryMapping.countDocuments(query);

    const wallet = await Wallet.findOne({ userId: franchise.userId });
    ...
    if (franchise.franchiseLevel === 'state') {
      subFranchisesCount = await Franchise.countDocuments({ parentFranchiseId: franchise._id });
      entrepreneursCount = await Entrepreneur.countDocuments({ state: franchise.state });
      const mappings = await TerritoryMapping.find({ stateFranchiseId: franchise._id });
      ...
      subFranchisesCount = await Franchise.countDocuments({ parentFranchiseId: franchise._id });
      entrepreneursCount = await Entrepreneur.countDocuments({ state: franchise.state, district: franchise.district });
      const mappings = await TerritoryMapping.find({ districtFranchiseId: franchise._id });
      vendorsCount = mappings.filter(m => m.businessType === 'vendor').length;
      spCount = mappings.filter(m => m.businessType === 'service_provider').length;
    } else if (franchise.franchiseLevel === 'mandal') {
      entrepreneursCount = await Entrepreneur.countDocuments({ state: franchise.state, district: franchise.district, mandal: franchise.mandal });
      const mappings = await TerritoryMapping.find({ mandalFranchiseId: franchise._id });
      vendorsCount = mappings.filter(m => m.businessType === 'vendor').length;
      spCount = mappings.filter(m => m.businessType === 'service_provider').length;
    }
...
export const getFranchiseNetwork = async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    const currentFranchise = await Franchise.findOne({
      userId: req.user.id,
    }).populate(
      "assignedTerritories",
      "level name state district mandal pincode parentId status"
    );
    ...
    const childEntrepreneurs = await Entrepreneur.find({
      $or: [
        { parentFranchiseId: currentFranchise._id },
    ...
    const mandalNodes = allTerritories.filter(
      (t: any) =>
        t.level === "Mandal" ||
        (t.district && t.mandal && !t.pincode)
    );

    const pincodeNodes = allTerritories.filter(
      (t: any) =>
        t.level === "Pincode" ||
        (t.district && t.mandal && t.pincode)
    );
    ...
    const network = {
      ...
      territories: allTerritories.map((t: any) => ({
        id: t._id,
        level:
          t.level ||
          (t.pincode
...
export const getFranchiseWallet = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
    ...
    const wallet = await WalletEngine.getOrCreateWallet(franchise.userId);
...
export const createFranchiseWithdrawal = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
    ...
    const updatedWallet = await WalletEngine.debit(franchise.userId, reqAmount, {
      category: 'Withdrawal',
      source: 'withdrawal',
...
export const getFranchiseApplications = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
...
export const handleFranchiseApplicationAction = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
    ...
    if (action === 'Approve') {
      ...
      let targetRole: any = 'vendor';
      if (type.includes("service")) targetRole = "service_provider";
      else if (type.includes("entrepreneur")) targetRole = "entrepreneur";
      ...
      if (app.state) {
        const stateRecord = await StateMaster.findOne({ name: { $regex: new RegExp(`^${app.state.trim()}$`, "i") } });
        if (stateRecord) {
          stateId = stateRecord._id;
          if (app.district) {
            const districtRecord = await DistrictMaster.findOne({ stateId: stateRecord._id, name: { $regex: new RegExp(`^${app.district.trim()}$`, "i") } });
            if (districtRecord) {
              districtId = districtRecord._id;
              if (app.mandal) {
                const mandalRecord = await MandalMaster.findOne({ stateId: stateRecord._id, districtId: districtRecord._id, name: { $regex: new RegExp(`^${app.mandal.trim()}$`, "i") } });
                if (mandalRecord) {
      ...
      const createBankDetails = (ap: any) => ({
      ...
      const createServiceProviderDocuments = (ap: any) => ({
        profilePhoto: "",
      ...
      if (targetRole === 'vendor') {
        const finalDocuments = [
          { id: "DOC-AD-F", name: "Aadhaar Front", status: "Approved" as const, url: app.documents?.aadhaar || "" },
          { id: "DOC-AD-B", name: "Aadhaar Back", status: "Not Uploaded" as const },
          { id: "DOC-PAN", name: "PAN Card", status: "Approved" as const, url: app.documents?.pan || "" },
          { id: "DOC-GST", name: "GST Certificate", status: "Approved" as const, url: app.documents?.gst || "" },
          { id: "DOC-LIC", name: "Business License", status: "Approved" as const, url: app.documents?.license || "" },
          { id: "DOC-BANK", name: "Bank Passbook/Cancelled Cheque", status: "Not Uploaded" as const },
          { id: "DOC-PROFILE", name: "Profile Photo", status: "Not Uploaded" as const },
        ];
        ...
        const stateFranchise = await Franchise.findOne({ franchiseLevel: "state", state: app.state, status: "active" });
        const districtFranchise = await Franchise.findOne({ franchiseLevel: "district", state: app.state, district: app.district, status: "active" });
        const mandalFranchise = await Franchise.findOne({ franchiseLevel: "mandal", state: app.state, district: app.district, mandal: app.mandal, status: "active" });
...
export const getAllFranchises: RequestHandler = async (req, res) => {
  try {
    const {
      franchiseLevel,
      state,
    ...
    if (franchiseLevel) filter.franchiseLevel = franchiseLevel;
    ...
    const franchises = await Franchise.find(filter)
      ...
      .populate(
        "assignedTerritories",
        "level name state district mandal pincode franchiseId"
      )
      .populate("approvedBy", "name email")
      .sort({ createdAt: -1 });
...
export const getFranchiseById: RequestHandler = async (req, res) => {
  try {
...
export const getFranchiseLogins = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
    ...
    const logins = await LoginAudit.find({ userId: req.user.id })
      .sort({ loginTime: -1 })
      .limit(10);
...
export const getFranchiseDashboardAnalytics = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
    ...
    const subFranchises = await Franchise.find({ parentFranchiseId: franchise._id });
    const entrepreneurs = await Entrepreneur.find(scopeFilter);
    const vendors = await Vendor.find(scopeFilter);
    ...
    const serviceProviders = await ServiceProvider.find(scopeFilter);
    const totalServiceProviders = serviceProviders.length;

    const deliveryPartnerUsers = await User.find({
      ...userScopeFilter,
    ...
    if (wallet && wallet.ledgerEntries) {
      wallet.ledgerEntries.forEach((entry: any) => {
    ...
    const scopedSettlements = await CommissionSettlement.find({
      orderId: { $in: orders.map(o => o._id) }
    });
    let vendorCommission = 0;
    let companyShare = 0;
    scopedSettlements.forEach(s => {
      if (s.settlementType === 'vendor') vendorCommission += s.amount;
      else if (s.settlementType === 'company') companyShare += s.amount;
    });

    const todayEarnings = wallet && wallet.ledgerEntries
      ? wallet.ledgerEntries.filter((e: any) => e.type?.toLowerCase() === 'credit' && new Date(e.createdAt || e.date || now) >= startOfToday).reduce((sum, e) => sum + e.amount, 0)
      : 0;

    const monthlyEarnings = wallet && wallet.ledgerEntries
      ? wallet.ledgerEntries.filter((e: any) => e.type?.toLowerCase() === 'credit' && new Date(e.createdAt || e.date || now) >= startOfMonth).reduce((sum, e) => sum + e.amount, 0)
      : 0;

    const totalLifetimeEarnings = wallet && wallet.ledgerEntries
      ? wallet.ledgerEntries.filter((e: any) => e.type?.toLowerCase() === 'credit').reduce((sum, e) => sum + e.amount, 0)
      : 0;

    // 10. Customer Growth
    const startOfWeek = new Date();
    startOfWeek.setDate(now.getDate() - 7);
    const newCustomersToday = await User.countDocuments({ ...userScopeFilter, roles: 'customer', createdAt: { $gte: startOfToday } });
    const newCustomersThisWeek = await User.countDocuments({ ...userScopeFilter, roles: 'customer', createdAt: { $gte: startOfWeek } });
    const newCustomersThisMonth = await User.countDocuments({ ...userScopeFilter, roles: 'customer', createdAt: { $gte: startOfMonth } });
    ...
    const withdrawRequests = wallet && wallet.ledgerEntries
      ? wallet.ledgerEntries.filter((e: any) => e.category === 'Withdrawal' && e.status === 'pending').length
      : 0;
    ...
    const scopedUsers = await User.find(userScopeFilter).select('_id');
    ...
    const districtSalesMap: Record<string, number> = {};
    const mandalSalesMap: Record<string, number> = {};
    orders.forEach(o => {
      const vend = vendors.find(v => v.userId.toString() === o.sellerId.toString());
      if (vend) {
        if (vend.district) districtSalesMap[vend.district] = (districtSalesMap[vend.district] || 0) + (o.totalAmount || 0);
        if (vend.mandal) mandalSalesMap[vend.mandal] = (mandalSalesMap[vend.mandal] || 0) + (o.totalAmount || 0);
      }
    });
    ...
    const totalReferrals = await Referral.countDocuments({ referrerUserId: franchise.userId });
    const referralsList = await Referral.find({ referrerUserId: franchise.userId });
        ...
        wallet.ledgerEntries.forEach((entry: any) => {
          if (entry.type?.toLowerCase() === 'credit' && entry.status !== 'rejected') {
            ...
            if (d.getMonth() === m.monthNum && d.getFullYear() === m.year) {
              const cat = entry.category || '';
              const amt = entry.amount || 0;
              if (["Product Commission", "product_commission"].includes(cat)) MLM += amt;
              else if (["Referral Bonus", "first_order_bonus", "first_purchase_product_commission"].includes(cat)) ReferralAmt += amt;
              else FranchiseComm += amt;
            }
          }
        });
      }
    ...
    // 15. Leaderboards
    let leaderboard = { title: "Leaderboard", type: "District", items: [] as any[] };
    const startOfCurrMonth = new Date(now.getFullYear(), now.getMonth(), 1);
    const startOfPrevMonth = new Date(now.getFullYear(), now.getMonth() - 1, 1);

    if (franchiseLevel === 'state') {
      const districtsList = await Franchise.find({ state, franchiseLevel: 'district' });
      const districtItems = [];
      for (const dist of districtsList) {
        const distVendors = await Vendor.find({ state, district: dist.district });
        const distVendorUserIds = distVendors.map(v => v.userId);
        const distOrders = await Order.find({ sellerId: { $in: distVendorUserIds }, orderStatus: { $nin: ['Cancelled', 'Payment Rejected'] } });
        ...
        } else if (currSales > 0) growth = '+100%';
      ...
      const mandalsList = await Franchise.find({ state, district, franchiseLevel: 'mandal' });
      const mandalItems = [];
      for (const man of mandalsList) {
        const manVendors = await Vendor.find({ state, district, mandal: man.mandal });
        const manVendorUserIds = manVendors.map(v => v.userId);
        const manOrders = await Order.find({ sellerId: { $in: manVendorUserIds }, orderStatus: { $nin: ['Cancelled', 'Payment Rejected'] } });
      ...
      for (const v of vendors) {
        const vOrders = await Order.find({ sellerId: v.userId, orderStatus: { $nin: ['Cancelled', 'Payment Rejected'] } });
...
export const getFranchiseCommissions = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
    ...
    const franchiseIds = settlements
      ...
      .filter(Boolean);
    const franchiseList = await Franchise.find({ _id: { $in: franchiseIds } });
    ...
    const referrals = await ReferralTransaction.find({ recipientUserId: franchise.userId })
...
export const getFranchiseReportsData = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
    ...
    const data = await ReportingService.getReportData(req.user.id, req.user.roles, reportType, criteria);
    const summary = await ReportingService.getSummaryMetrics(req.user.id, req.user.roles, criteria);
...
export const downloadFranchiseReport = async (req: any, res: Response): Promise<void> => {
  try {
    ...
    const data = await ReportingService.getReportData(user._id.toString(), user.roles, reportType, criteria);
    await ExportEngine.exportReport(res, reportType, tf, data, exportFormat);
  } catch (error: any) {
...
export const getFranchiseCustomers = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
...
export const getFranchiseDeliveryPartners = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
    ...
    const { state, district, mandal, franchiseLevel } = franchise;
    const filter: any = { roles: 'delivery_partner' };
    ...
    const dps = await DeliveryPartner.find({ userId: { $in: userIds } }).sort({ createdAt: -1 });

    const deliveryPartners = await Promise.all(dps.map(async (dp) => {
      const wallet = await Wallet.findOne({ userId: dp.userId });
      const walletBalance = wallet ? wallet.availableBalance : 0;
      const payoutTotal = wallet ? wallet.withdrawnBalance : 0;

      const assigned = await Order.countDocuments({ deliveryAgentId: dp.userId.toString() });
      const completed = await Order.countDocuments({ deliveryAgentId: dp.userId.toString(), orderStatus: 'Delivered' });
      const failed = await Order.countDocuments({ deliveryAgentId: dp.userId.toString(), orderStatus: { $in: ['Cancelled', 'Returned'] } });
...
export const getFranchiseTerritoryDetails = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
    ...
    if (level === 'mandal') {
      ...
      res.status(200).json({
        ...
        district: districtName,
        mandal: mandalName,
        entrepreneurs: entrepreneurs.map((e: any) => ({
          _id: e._id,
          name: e.name,
          mobile: e.mobile,
          phone: e.mobile,
