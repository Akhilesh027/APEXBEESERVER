...
const parseTime = (tStr: string): number => {
  if (!tStr || tStr.toLowerCase() === "closed") return 0;
...
export const getDashboardStats = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const bookings = await ServiceRequest.find({ providerId });
    const todayStr = new Date().toISOString().split("T")[0];

    const todayBookings = bookings.filter((b) => b.bookingDate === todayStr);
    const completedToday = todayBookings.filter((b) => b.status === "Completed").length;
    const upcomingBookings = bookings.filter(
      (b) => new Date(b.bookingDate).getTime() > new Date(todayStr).getTime() && b.status !== "Cancelled"
    ).length;
...
export const getBookings = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const bookings = await ServiceRequest.find(filterQuery)
...
export const getBookingById = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!booking) {
      ...
      const byCode = await ServiceRequest.findOne({ bookingCode: id })
        .populate("customerId", "name email phone profilePhoto")
        .populate("providerId", "name email phone");
      if (!byCode) {
...
export const updateBookingStatus = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    let booking = await ServiceRequest.findById(id);
    if (!booking) {
      booking = await ServiceRequest.findOne({ bookingCode: id });
    }
    ...
    if (status === "Completed") {
      await SettlementEngine.processServiceBookingSettlement(booking._id, session);
    }

    await session.commitTransaction();

    // Reload booking to get the final settlement state
    const finalBooking = await ServiceRequest.findById(booking._id);
...
export const getAvailableSlots = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const provider = await ServiceProvider.findOne({ userId: String(providerId) });
    if (!provider) {
    ...
    for (let h = startHour; h + 1.5 <= endHour; h += 2) {
      ...
      if (slotEnd > breakStart && slotEnd <= breakEnd) continue;
...
export const payBooking = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (paymentMethod === "wallet") {
      ...
      const customerWallet = await WalletEngine.getOrCreateWallet(customerId, session);
