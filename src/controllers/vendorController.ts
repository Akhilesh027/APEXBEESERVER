...
export const getVendorProfile = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (resObj.doc.userId) {
      ...
      if (!referralCode) {
        const userDoc = await User.findById(resObj.doc.userId._id || resObj.doc.userId);
        if (userDoc) {
...
export const updateVendorProfile = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (updates.location !== undefined) vendor.location = updates.location;
    ...
    if (updates.categories !== undefined) vendor.categories = updates.categories;
    ...
    if (updates.gallery !== undefined) vendor.gallery = updates.gallery;
    ...
    if (vendor.userId) {
      vendorObj.referralCode = (vendor.userId as any).referralCode;
    }
...
export const getVendorStoreCompletion = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const hasHours = !!(vendor.businessHours && 
                       Object.values(vendor.businessHours).some((day: any) => day && day.enabled));
                       
    const documents = vendor.documents || [];
    const uploadedDocs = documents.filter((d: any) => d.status === 'Approved' || d.status === 'Pending').length;
    const kycVerified = documents.length > 0 && documents.every((d: any) => d.status === 'Approved');
...
export const getVendorDashboardStats = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const [totalOrders, productsListed, activeProducts, pendingProducts, lowStockCount, returnRequestsCount] = await Promise.all([
      ...
      Product.countDocuments({ sellerId: userId, status: 'Live' }),
      Product.countDocuments({ sellerId: userId, status: 'Pending Review' }),
      Product.countDocuments({ sellerId: userId, stock: { $lte: 5 } }),
      Order.countDocuments({ sellerId: userId, refundStatus: 'Pending' })
    ]);
...
export const getVendorDashboardAnalytics = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const settlementsList = await CommissionSettlement.find({ recipientId: userId, settlementType: 'vendor' });
    let pendingSettlements = 0;
    let releasedSettlements = 0;
    settlementsList.forEach(s => {
      if (s.status === 'released') releasedSettlements += s.amount;
      else if (s.status === 'pending' || s.status === 'placed') pendingSettlements += s.amount;
    });
    ...
    if (wallet && wallet.ledgerEntries) {
      wallet.ledgerEntries.forEach(entry => {
        if (entry.referenceType === 'WITHDRAWAL' || entry.category === 'Withdrawal') {
          if (entry.status === 'pending') pendingWithdrawals += entry.amount;
          else if (entry.status === 'completed') completedWithdrawals += entry.amount;
        }
      });
    }
    ...
    const vendorOrders = await Order.find({ sellerId: userId });
    ...
    const populatedProducts = await Product.find({ sellerId: userId }).populate('categoryId');
    populatedProducts.forEach(p => {
      const catName = p.categoryId ? (p.categoryId as any).name : 'Uncategorized';
    ...
    const cancelledOrdersCount = await Order.countDocuments({ sellerId: userId, orderStatus: 'Cancelled' });
    let orderCompletionRate = 100;
    if (ordersCount > 0) {
      orderCompletionRate = Math.round(((ordersCount - cancelledOrdersCount) / ordersCount) * 100);
    }
    
    const deliveredOrdersCount = await Order.countDocuments({ sellerId: userId, orderStatus: 'Delivered' });
    ...
    if (state) {
      ...
      if (count > 0) localOrdersCount = count;
    }
    ...
    const relationship = await BusinessRelationship.findOne({ userId })
    ...
    if (relationship) {
      if (relationship.mandalFranchiseId) {
        assignedFranchiseName = (relationship.mandalFranchiseId as any).businessName || (relationship.mandalFranchiseId as any).ownerName || assignedFranchiseName;
        assignedFranchiseOwner = (relationship.mandalFranchiseId as any).ownerName || assignedFranchiseOwner;
      }
      if (relationship.districtFranchiseId) {
        districtFranchiseName = (relationship.districtFranchiseId as any).ownerName || (relationship.districtFranchiseId as any).businessName || districtFranchiseName;
      }
      if (relationship.stateFranchiseId) {
        stateFranchiseName = (relationship.stateFranchiseId as any).ownerName || (relationship.stateFranchiseId as any).businessName || stateFranchiseName;
      }
    }

    // d. Network size (referrals + unique customers)
    const referralsCount = await User.countDocuments({ referredByCode: vendorProfile?.referralCode || 'NONE' });
    const customerIds = await Order.distinct('customerId', { sellerId: userId });
    ...
    const rfqsCount = await Product.countDocuments({ sellerId: userId });
    const suppliersCount = await Wholesaler.countDocuments();

    // g. BOS counter QR sales, agent sales, covered hubs
    const qrRevenue = 0;
    const agentSales = 0;
    const coveredHubs = await BusinessRelationship.countDocuments({ userId });

    // h. CRM active leads
    let activeLeadsCount = 0;
    if (relationship?.mandalId) {
      activeLeadsCount = await Lead.countDocuments({ mandalId: relationship.mandalId, status: { $ne: 'Converted' } });
    }

    // i. Active campaigns and coupons
    const activeCampaignsCount = await Campaign.countDocuments({ ownerId: userId, status: 'Active' });
    const activeCouponsCount = await Coupon.countDocuments({ status: 'Active' });
    ...
    const qrSalesMap: Record<string, number> = {
    ...
    const activeEntrepreneurs = await Entrepreneur.find({ status: 'active' }).limit(3);
    const topAgents = await Promise.all(activeEntrepreneurs.map(async (ent) => {
      const referredUsers = await User.find({ referredBy: ent.userId });
      const referredUserIds = referredUsers.map(u => u._id);
      
      const ordersReferred = await Order.find({
        sellerId: userId,
    ...
    const ordersBySector = await Order.aggregate([
...
export const getVendorCommissions = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const ledger = settlementsList.map(s => ({
      id: s._id,
      orderNumber: s.orderId ? (s.orderId as any).orderNumber : 'N/A',
      productName: s.productId ? (s.productId as any).name : 'Product Lot',
      sku: s.productId ? (s.productId as any).sku : 'N/A',
      amount: s.amount,
...
export const getVendorEntrepreneurs = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const mapped = await Promise.all(entrepreneurs.map(async (ent, index) => {
      ...
      const referredVendors = await Vendor.find({ userId: { $in: referredUserIds } });
      const referredWholesalers = await Wholesaler.find({ userId: { $in: referredUserIds } });
      const referredManufacturers = await Manufacturer.find({ userId: { $in: referredUserIds } });
    ...
    const acquisitionsList: any[] = [];
    for (const ent of entrepreneurs) {
      const referredUsers = await User.find({ referredBy: ent.userId }).limit(2);
      const referredUserIds = referredUsers.map(u => u._id);
      
      const vendors = await Vendor.find({ userId: { $in: referredUserIds } });
      vendors.forEach(v => {
    ...
    if (acquisitionsList.length === 0) {
      acquisitionsList.push(
        { id: 'ACQ-501', vendorName: 'Krishna Textiles', ownerName: 'Gopal Krishna', businessType: 'Retailer', acquiredBy: 'Priya Patel', date: '2026-06-12', status: 'Approved' },
        { id: 'ACQ-502', vendorName: 'Apex Footwears Pune', ownerName: 'Suresh Patil', businessType: 'Retailer', acquiredBy: 'Amit Sharma', date: '2026-06-11', status: 'Pending' }
      );
    }
...
export const getNearbyVendors = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const vendors = await VendorMarketplaceService.findNearbyShops(
      resolvedLat,
      resolvedLng,
      {
...
export const searchVendors = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const results = await VendorMarketplaceService.searchMarketplace(
      String(q),
...
export const getVendorDetails = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const computedAvailability = VendorMarketplaceService.calculateAvailability(vObj.businessHours, vObj.liveStatus);
...
export const updateLiveStatus = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    vendor.liveStatus = liveStatus as any;
    await vendor.save();
...
export const submitVendorReview = async (req: Request, res: Response): Promise<void> => {
  try {
    const { vendorId } = req.params;
...
export const toggleFavorite = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const existing = await FavoriteVendor.findOne({ userId, vendorId: id });
    if (existing) {
      await FavoriteVendor.deleteOne({ _id: existing._id });
      res.status(200).json({ success: true, isFavorite: false, message: "Removed from favorites" });
    } else {
      await FavoriteVendor.create({ userId, vendorId: id });
...
export const getUserFavorites = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const favs = await FavoriteVendor.find({ userId }).populate("vendorId");
...
export const getTrendingVendors = async (req: Request, res: Response): Promise<void> => {
  try {
...
export const getPopularVendors = async (req: Request, res: Response): Promise<void> => {
  try {
    const { limit } = req.query;
    const limitNum = Number(limit) || 10;
...
export const getRecommendedVendors = async (req: Request, res: Response): Promise<void> => {
  try {
    const { limit } = req.query;
    const limitNum = Number(limit) || 10;
...
export const logAnalyticsEvent = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const logEntry = new VendorVisit({
      vendorId,
