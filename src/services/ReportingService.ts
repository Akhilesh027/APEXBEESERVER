...
export class ReportingService {
   ...
   */
  static async resolveScopes(userId: string, roles: string[], criteria: any = {}): Promise<ScopeFilters> {
    ...
    // Date Range Builder
    const buildDateFilter = (field: string = 'createdAt') => {
    ...
    const applyHierarchyFilters = (target: any) => {
      if (criteria.state) target.state = criteria.state;
      if (criteria.district) target.district = criteria.district;
      if (criteria.mandal) target.mandal = criteria.mandal;
    };
    ...
    if (roles.includes('admin')) {
      ...
      if (criteria.state || criteria.district || criteria.mandal) {
        const geoQuery: any = {};
        if (criteria.state) geoQuery.state = criteria.state;
        if (criteria.district) geoQuery.district = criteria.district;
        if (criteria.mandal) geoQuery.mandal = criteria.mandal;
        const matchingVendors = await Vendor.find(geoQuery).select('userId');
    ...
    if (isState || isDistrict || isMandal) {
      ...
      if (franchise) {
        ...
        let geoFilter: any = {};
        if (franchiseLevel === 'state') geoFilter = { state };
        else if (franchiseLevel === 'district') geoFilter = { state, district };
        else geoFilter = { state, district, mandal };

        // Scope validation override
        if (criteria.state && criteria.state !== state) geoFilter.state = 'UNAUTHORIZED';
        if (criteria.district && district && criteria.district !== district) geoFilter.district = 'UNAUTHORIZED';
        if (criteria.mandal && mandal && criteria.mandal !== mandal) geoFilter.mandal = 'UNAUTHORIZED';
        ...
        if (district) scope.entrepreneurs.district = district;
        if (mandal) scope.entrepreneurs.mandal = mandal;

        scope.users.roles = 'customer';
        scope.users['territory.state'] = state;
        if (district) scope.users['territory.district'] = district;
        if (mandal) scope.users['territory.mandal'] = mandal;

        const vendors = await Vendor.find(geoFilter).select('userId');
    ...
    if (roles.includes('entrepreneur')) {
      ...
      const ent = await Entrepreneur.findOne({ userId });
      if (ent) {
        const onboardedVendors = await Vendor.find({ entrepreneurId: ent._id }).select('userId');
        const onboardedUsers = await User.find({ referredBy: userId }).select('_id');
        scope.orders.$or = [
   ...
   */
  static async getSummaryMetrics(userId: string, roles: string[], criteria: any = {}): Promise<any> {
    ...
    const sumRevenue = async (match: any) => {
    ...
    // Orders breakdown
    const totalOrders = await Order.countDocuments(scopes.orders);
    const completedOrders = await Order.countDocuments({ ...scopes.orders, orderStatus: 'Delivered' });
    const pendingOrders = await Order.countDocuments({ ...scopes.orders, orderStatus: { $in: ['Placed', 'Confirmed', 'Packed', 'Shipped'] } });
    const cancelledOrders = await Order.countDocuments({ ...scopes.orders, orderStatus: 'Cancelled' });

    // Downline stats
    const totalCustomers = await User.countDocuments(scopes.users);
    const activeVendors = await Vendor.countDocuments({ ...scopes.vendors, status: 'active' });
    const activeEnts = await Entrepreneur.countDocuments({ ...scopes.entrepreneurs, status: 'active' });
    const activeRiders = await DeliveryPartner.countDocuments({ ...scopes.deliveryPartners, status: 'active' });

    // Payout and financials
    const wallet = await Wallet.findOne({ userId });
    const walletBal = wallet?.availableBalance || 0;

    const commissionRes = await CommissionSettlement.aggregate([
    ...
    const referralRes = await ReferralTransaction.aggregate([
    ...
    const mlmRes = await CommissionSettlement.aggregate([
    ...
    // Subscriptions MRR
    const subRes = await LocalShopSubscription.aggregate([
    ...
    const companyRes = await CommissionSettlement.aggregate([
    ...
    if (avgDeliveryTime <= 0) avgDeliveryTime = 32;

    // 4. Scanned critical operational alerts
    const gstMissingCount = await Vendor.countDocuments({ ...scopes.vendors, gstNumber: { $in: ['', null] } });
    const pendingSettlementsCount = await CommissionSettlement.countDocuments({ ...scopes.settlements, status: 'pending' });
   ...
   */
  static async getReportData(userId: string, roles: string[], type: string, criteria: any = {}): Promise<any[]> {
    const scopes = await this.resolveScopes(userId, roles, criteria);

    switch (type) {
      case 'commission': {
        const settlements = await CommissionSettlement.find(scopes.settlements)
          .populate('vendorId')
          .sort({ createdAt: -1 })
          .limit(100);
        return settlements.map(s => {
          const v = s.vendorId as any;
          return {
            id: s._id.toString(),
      ...
      case 'referral': {
        const referrals = await ReferralTransaction.find(scopes.referrals)
          .populate('referredUserId')
          .sort({ createdAt: -1 })
          .limit(100);
        return referrals.map(r => {
          const u = r.referredUserId as any;
          return {
      ...
      case 'mlm': {
        // MLM payout flows
        const mlm = await CommissionSettlement.find({ ...scopes.settlements, settlementType: 'franchise' })
          .populate('orderId')
          .sort({ createdAt: -1 })
          .limit(100);
        return mlm.map(m => ({
      ...
      case 'vendor': {
        const vendors = await Vendor.find(scopes.vendors).limit(50);
        const list = [];
        for (const v of vendors) {
          const salesRes = await Order.aggregate([
      ...
      case 'customer': {
        const customers = await User.find(scopes.users).limit(50);
        const list = [];
        for (const c of customers) {
          const orderCount = await Order.countDocuments({ customerId: c._id });
          const spendsRes = await Order.aggregate([
      ...
      case 'entrepreneur': {
        const ents = await Entrepreneur.find(scopes.entrepreneurs).populate('userId').limit(50);
      ...
      case 'service': {
        const spList = await ServiceProvider.find(scopes.serviceProviders).limit(50);
        const list = [];
        for (const sp of spList) {
          const count = await mongoose.model("ServiceRequest").countDocuments({ providerId: sp._id });
          const earningsRes = await mongoose.model("ServiceRequest").aggregate([
      ...
      case 'delivery': {
        const riders = await DeliveryPartner.find(scopes.deliveryPartners).limit(50);
        return riders.map(r => ({
