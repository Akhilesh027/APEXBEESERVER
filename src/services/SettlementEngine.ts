...
export class SettlementEngine {
  ...
  static readonly COMPANY_ID = new mongoose.Types.ObjectId('660000000000000000000001');
  static readonly WISHLINK_ID = new mongoose.Types.ObjectId('660000000000000000000002');
  static readonly REFERRAL_POOL_ID = new mongoose.Types.ObjectId('660000000000000000000003');

  static async createReferralTransactionUnique(doc: any, session?: ClientSession) {
    const existing = await ReferralTransaction.findOne({
  ...
  static async createCommissionSettlementUnique(doc: any, session?: ClientSession) {
   ...
   */
  static async ensureSystemProfiles(session?: ClientSession) {
    const systems = [
    ...
    for (const sys of systems) {
      let query = User.findById(sys.id);
   ...
   */
  static async createSettlements(order: any, session?: ClientSession): Promise<void> {
    await this.ensureSystemProfiles(session);

    // Get customer profile
    let queryCustomer = User.findById(order.customerId);
    if (session) queryCustomer = queryCustomer.session(session);
    ...
    if (order.items && order.items.length > 0) {
      ...
      let queryProducts = Product.find({ _id: { $in: productIds } });
      if (session) queryProducts = queryProducts.session(session);
      const products = await queryProducts;
      const periods = products.map((p: any) => p.returnPeriodDays ?? 7);
      if (periods.length > 0) {
    ...
    // ==========================================
    let queryCount = Order.countDocuments({
      customerId: customer._id,
      _id: { $ne: order._id },
      orderStatus: { $nin: ['Cancelled', 'Returned'] }
    });
    if (session) queryCount = queryCount.session(session);
    const successfulOrdersCount = await queryCount;

    if (!customer.firstOrderQualified && successfulOrdersCount === 0) {
      let queryFirstOrderTx = ReferralTransaction.findOne({
    ...
    if (order.items && order.items.length > 0) {
      for (const item of order.items) {
        let queryProd = Product.findById(item.productId);
        if (session) queryProd = queryProd.session(session);
        ...
        let queryRel = BusinessRelationship.findOne({
          userId: product.sellerId,
          businessType: { $in: ["vendor", "manufacturer", "wholesaler"] },
          status: "active"
        });
        if (session) queryRel = queryRel.session(session);
   ...
   */
  static async pendSettlements(orderId: any, session?: ClientSession): Promise<void> {
    ...
    try {
      ...
      const deliveredOrder = await Order.findById(orderId).lean();
      if (deliveredOrder && (deliveredOrder as any).items && (deliveredOrder as any).items.length > 0) {
        const productIds = (deliveredOrder as any).items
          .map((item: any) => item.productId)
          .filter(Boolean);
        if (productIds.length > 0) {
          const prods = await Product.find({ _id: { $in: productIds } }).lean();
          const periods = prods.map((p: any) => Number(p.returnPeriodDays) || 7);
          if (periods.length > 0) maxReturnPeriodDays = Math.max(...periods);
        }
      }
    } catch (rdErr) {
    ...
    let queryPlacedTxs = ReferralTransaction.find({ orderId, status: "placed" });
    if (session) queryPlacedTxs = queryPlacedTxs.session(session);
    ...
    let queryPlacedSettlements = CommissionSettlement.find({ orderId, status: "placed" });
    if (session) queryPlacedSettlements = queryPlacedSettlements.session(session);
  ...
  static async releaseEligibleSettlements(
    sessionOverride?: ClientSession,
    forceOrderId?: string | mongoose.Types.ObjectId,
    adminId?: string | mongoose.Types.ObjectId
  ): Promise<{ releasedTxs: number; releasedSettlements: number }> {
    ...
    const executeBlock = async (sess: ClientSession) => {
      ...
      const pendingTxs = await ReferralTransaction.find(txQuery).session(sess);
      ...
      const pendingSettlements = await CommissionSettlement.find(settlementQuery).session(sess);
      ...
      if (forceOrderId && (releasedTxs > 0 || releasedSettlements > 0)) {
        const order = await Order.findById(forceOrderId).session(sess);
   ...
   */
  static async cancelSettlements(orderId: any, session?: ClientSession): Promise<void> {
    // 1. Cancel Referral Transactions
    let queryTxs = ReferralTransaction.find({
      orderId,
      status: { $in: ["placed", "pending"] }
    });
    if (session) queryTxs = queryTxs.session(session);
    const txs = await queryTxs;

    for (const tx of txs) {
      if (tx.status === "pending") {
        // FIX 4: Use tx._id as referenceId for reverse (must match the hold entry referenceId)
        await WalletEngine.reverse(tx.recipientUserId, tx.amount, {
          category: "Referral Bonus",
          source: tx.transactionType,
    ...
    if (hasFirstOrderBonus) {
      ...
      if (order) {
        let queryCust = User.findById(order.customerId);
        if (session) queryCust = queryCust.session(session);
    ...
    let querySettlements = CommissionSettlement.find({
      orderId,
      status: { $in: ["placed", "pending"] }
    });
    if (session) querySettlements = querySettlements.session(session);
   ...
   */
  static async processServiceBookingSettlement(
    bookingId: any,
    sessionOverride?: ClientSession
  ): Promise<void> {
    const session = sessionOverride || (await mongoose.startSession());
    ...
    const executeBlock = async (sess: ClientSession) => {
      const booking = await ServiceRequest.findById(bookingId).session(sess);
      if (!booking) return;

      // If already settled, do not repeat
      if (booking.paymentDetails && booking.paymentDetails.status === "Approved") return;

      const provider = await ServiceProvider.findOne({ userId: booking.providerId }).session(sess);
      if (!provider) return;
      ...
      // Resolve actual User IDs for the recipients
      const stateFranchise = provider.stateFranchiseId ? await mongoose.model("Franchise").findById(provider.stateFranchiseId).session(sess) : null;
      const districtFranchise = provider.districtFranchiseId ? await mongoose.model("Franchise").findById(provider.districtFranchiseId).session(sess) : null;
      const mandalFranchise = provider.mandalFranchiseId ? await mongoose.model("Franchise").findById(provider.mandalFranchiseId).session(sess) : null;
      const entrepreneur = provider.entrepreneurId ? await mongoose.model("Entrepreneur").findById(provider.entrepreneurId).session(sess) : null;
      ...
      const createServiceSettlement = async (recipientId: any, amount: number, type: string) => {
