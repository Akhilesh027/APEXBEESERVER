...
export class ExportEngine {
   ...
   */
  static async exportReport(res: Response, type: string, timeframe: string, data: any[], format: string): Promise<void> {
    ...
    if (format === 'pptx') {
      ...
      if (data.length > 0) {
        const headers = Object.keys(data[0]);
        const rows: pptxgen.TableRow[] = [headers.map(h => ({ text: h.toUpperCase() }))];
