...
export class VendorMarketplaceService {
  ...
  static async findNearbyShops(
    ...
    let rawVendors: any[] = [];
    let searchMode: 'gps' | 'pincode' | 'city' = 'gps';
    ...
    if (lat && lng) {
      ...
      let sortStage: any = { $sort: { distanceInKm: 1 } };
      if (options.sort === "highest_rated") sortStage = { $sort: { "rating.average": -1, distanceInKm: 1 } };
      else if (options.sort === "fastest_delivery") sortStage = { $sort: { estimatedDeliveryMinutes: 1, distanceInKm: 1 } };
      else if (options.sort === "lowest_delivery_fee") sortStage = { $sort: { deliveryCharge: 1, distanceInKm: 1 } };
      else if (options.sort === "trending") sortStage = { $sort: { "rating.totalReviews": -1, distanceInKm: 1 } };

      pipeline.push(sortStage);
      pipeline.push({ $limit: limitNum });

      rawVendors = await Vendor.aggregate(pipeline);
    }
