...
export class NotificationService {
   ...
   */
  static compileTemplate(templateStr: string, data: Record<string, any>): string {
   ...
   */
  static isQuietHoursActive(preference: any): boolean {
    if (!preference || !preference.quietHours?.enabled) return false;
  ...
  static async sendNotification(
    ...
    try {
      const recIdObj = new mongoose.Types.ObjectId(recipientId.toString());

      // 1. Fetch template
      const template = await NotificationTemplate.findOne({ eventCode, isActive: true });
      if (!template) {
      ...
      const recipient = await User.findById(recIdObj);
      if (!recipient) {
        console.warn(`[NotificationService] Recipient user not found: ${recipientId}`);
        return false;
      }

      let pref = await NotificationPreference.findOne({ userId: recIdObj });
      if (!pref) {
        // Fallback: create default preferences document
        pref = new NotificationPreference({ userId: recIdObj });
        await pref.save();
      }

      // Check category toggle
      const categoryKey = template.category;
      if (pref.categories && (pref.categories as any)[categoryKey] === false) {
      ...
      const notification = new Notification({
        ...
        entityId: payload.entityId ? new mongoose.Types.ObjectId(payload.entityId.toString()) : undefined,
        title,
        message,
        icon,
        image: payload.image || '',
      ...
      if (pref.channels.inApp) {
        try {
          sendRealtimeNotification(recIdObj.toString(), notification);
  ...
  private static async dispatchEmail(recipient: any, template: any, payload: Record<string, any>, notification: any) {
    try {
...
------------------------------------------------------`);
  ...
  private static async dispatchPush(recipient: any, template: any, payload: Record<string, any>, notification: any) {
    try {
  ...
  private static async dispatchWhatsApp(recipient: any, template: any, payload: Record<string, any>, notification: any) {
    try {
      const phone = recipient.phone || recipient.mobile || 'Unknown';
...
======================================================`);
  ...
  private static resolveRecipientType(roles: string[]): any {
    if (roles.includes('admin')) return 'User';
    if (roles.some(r => r.includes('franchise'))) return 'Franchise';
    if (roles.includes('vendor')) return 'Vendor';
    if (roles.includes('wholesaler')) return 'Wholesaler';
    if (roles.includes('manufacturer')) return 'Manufacturer';
    if (roles.includes('service_provider')) return 'ServiceProvider';
    if (roles.includes('delivery_partner')) return 'DeliveryPartner';
    return 'User';
  }
}
