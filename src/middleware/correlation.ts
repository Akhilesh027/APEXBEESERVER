...
export const correlationMiddleware = (req: CorrelatedRequest, res: Response, next: NextFunction) => {
  const startTime = Date.now();

  // 1. Establish request correlation ID
  const headerReqId = req.headers['x-request-id'] || req.headers['request-id'];
  const requestId = Array.isArray(headerReqId) ? headerReqId[0] : (headerReqId as string) || randomUUID();
  
  // Set response header for client tracking
  res.setHeader('x-request-id', requestId);

  // 2. Extract correlation attributes
  const userId = (req as any).user?.id || (req as any).user?._id || req.headers['x-user-id'] || req.body?.userId || req.body?.customerId;
  ...
  req.log = (level: 'info' | 'warn' | 'error', msg: string, ...meta: any[]) => {
    const ctx = req.correlation!;
    const timestamp = new Date().toISOString();
    
    // Structured JSON format for production environments
    if (env.NODE_ENV === 'production' || process.env.STRUCTURED_LOGGING === 'true') {
      const logObject = {
        timestamp,
