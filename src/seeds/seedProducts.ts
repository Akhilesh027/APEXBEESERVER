...
export const seedCatalog = async (dryRun = false) => {
  ...
  const variantsPath = path.join(__dirname, 'data', 'variants.json');

  const rawCats = JSON.parse(fs.readFileSync(categoriesPath, 'utf8'));
  const rawSubs = JSON.parse(fs.readFileSync(subcategoriesPath, 'utf8'));
  const rawProds = JSON.parse(fs.readFileSync(productsPath, 'utf8'));
  const rawVars = JSON.parse(fs.readFileSync(variantsPath, 'utf8'));
  ...
  for (const prod of rawProds) {
    ...
    const subRaw = rawSubs.find((s: any) => s.seedKey === prod.subcategorySeedKey);
    ...
    const doc = await Product.create({
      ...
      isActive: true,
      createdBy: new mongoose.Types.ObjectId(), // placeholder creator
    });
  ...
  for (const vr of rawVars) {
    ...
    if (vr.sku.includes('MILK')) { mrp = 60; sellingPrice = 54; }
    else if (vr.sku.includes('WATER')) { mrp = 80; sellingPrice = 65; }
    else if (vr.sku.includes('TOMATO')) { mrp = 40; sellingPrice = 32; }
    else if (vr.sku.includes('SAREE')) { mrp = 4999; sellingPrice = 3999; }
    else if (vr.sku.includes('BOAT')) { mrp = 1999; sellingPrice = 1299; }
    else if (vr.sku.includes('JAS-FLW-100')) { mrp = 50; sellingPrice = 40; }
    else if (vr.sku.includes('JAS-FLW-250')) { mrp = 120; sellingPrice = 90; }
