...
export async function testProductIsolation(tokens: Record<string, string>, host: string, userIds: Record<string, string>) {
  console.log(' - Running Product Isolation Tests...');

  // Set up mock product for Vendor A
  const vendorAProduct = new Product({
    sellerId: new mongoose.Types.ObjectId(userIds.vendorA), // Vendor A's User ID
  ...
  const vendorALiveProduct = new Product({
    sellerId: new mongoose.Types.ObjectId(userIds.vendorA), // Vendor A's User ID
  ...
  try {
    ...
    assert.strictEqual(resDraft.status, 404, "Vendor B should get 404 when reading Vendor A's Draft product");
    ...
    assert.strictEqual(resEdit.status, 404, "Vendor B should get 404 when editing Vendor A's product");
    ...
    assert.strictEqual(resDelete.status, 404, "Vendor B should get 404 when deleting Vendor A's product");
    ...
    assert.strictEqual(resProtected.status, 400, "Vendor A should get 400 when trying to edit protected fields");
    ...
    assert.strictEqual(resLive.status, 200, "Vendor B should be able to read Vendor A's Live product");
    const liveBody = await resLive.json() as any;
    assert.ok(liveBody.product, "Live product payload should exist");
    assert.strictEqual(liveBody.product.adminPricing, undefined, "Sensitive fields like adminPricing must be projected away");
    assert.strictEqual(liveBody.product.commissionShares, undefined, "Sensitive fields like commissionShares must be projected away");

  } finally {
    // Cleanup
    await Product.deleteOne({ _id: vendorAProduct._id });
    await Product.deleteOne({ _id: vendorALiveProduct._id });
  }
