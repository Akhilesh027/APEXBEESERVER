...
const _orderModel = Order.modelName;
const _referralModel = Referral.modelName;
const _entrepreneurModel = Entrepreneur.modelName;
...
async function run() {
  ...
  for (const s of settlements) {
    ...
    const franchise = await Franchise.findById(s.recipientId);
    if (franchise) {
      ...
      const entrepreneur = await Entrepreneur.findById(s.recipientId);
      if (entrepreneur) {
        targetUserId = (entrepreneur as any).userId;
        console.log(`Updating settlement ${s._id} recipient from Entrepreneur ${entrepreneur._id} to User ${(entrepreneur as any).userId}`);
      }
    }
    ...
    if (targetUserId) {
      ...
      if (s.stateFranchiseId) {
        const f = await Franchise.findById(s.stateFranchiseId);
        if (f) s.stateFranchiseId = f.userId;
      }
      if (s.districtFranchiseId) {
        const f = await Franchise.findById(s.districtFranchiseId);
        if (f) s.districtFranchiseId = f.userId;
      }
      if (s.mandalFranchiseId) {
        const f = await Franchise.findById(s.mandalFranchiseId);
        if (f) s.mandalFranchiseId = f.userId;
      }
      if (s.entrepreneurId) {
        const e = await Entrepreneur.findById(s.entrepreneurId);
        if (e) s.entrepreneurId = (e as any).userId;
      }
      await s.save();
    }
  }
  ...
  for (const w of wallets) {
    ...
    const franchise = await Franchise.findById(w.userId);
    if (franchise) {
      actualUserId = franchise.userId;
    } else {
      // Check if wallet userId is an Entrepreneur ID
      const entrepreneur = await Entrepreneur.findById(w.userId);
      if (entrepreneur) {
        actualUserId = (entrepreneur as any).userId;
      }
    }
    ...
    if (actualUserId) {
      ...
      let userWallet = await Wallet.findOne({ userId: actualUserId });
      if (!userWallet) {
  ...
  const referredUserId = new mongoose.Types.ObjectId("6a44c6901b05eb2b5d69cf45");
  ...
  const updatedWallets = await Wallet.find().populate('userId', 'name email roles');
  console.log('\n--- UPDATED WALLETS ---');
