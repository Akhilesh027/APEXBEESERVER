...
const unsplash = (query, w = 800, h = 600) => `https://images.unsplash.com/photo-${query}?auto=format&fit=crop&w=${w}&h=${h}&q=80`;
...
async function seedCatalog() {
    ...
    const vendor = await User_1.User.findOne({ roles: 'vendor' }).lean();
    if (!vendor) {
    ...
    await Category_1.default.deleteMany({ slug: { $in: CAT_DATA.map(c => c.slug) } });
    const slugsAll = CAT_DATA.flatMap(c => [c.slug, ...c.subs.map(s => s.slug)]);
    await Category_1.default.deleteMany({ slug: { $in: slugsAll } });
    await Product_1.default.deleteMany({ sku: /^DEMO-SEED-/ });
    ...
    const parents = await Category_1.default.insertMany(parentDocs);
    console.log(`Inserted ${parents.length} parent categories.`);
    // Insert 20 subcategories (2 per parent)
    const subDocs = CAT_DATA.flatMap((c, i) => c.subs.map((s, j) => ({
        name: s.name,
        slug: s.slug,
        image: s.image,
        level: 2,
    ...
    const subs = await Category_1.default.insertMany(subDocs);
    ...
    await Product_1.default.insertMany(productDocs);
