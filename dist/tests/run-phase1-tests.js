...
async function runPhase1Tests() {
    ...
    await User_1.User.deleteMany({ email: /@phase1test\.com$/ });
    await Product_1.default.deleteMany({ sku: /^SKU-PHASE1/ });
    await Order_1.Order.deleteMany({ orderNumber: /^AB-PHASE1/ });
    ...
    try {
        ...
        const pricingResult = await pricingService_1.PricingService.calculateCheckoutPricing([{ productId: product._id.toString(), quantity: 1 }], undefined);
        // subtotal = 100, shippingFee = 10, packingFee = 5. grandTotal = 115.
        assert_1.default.strictEqual(pricingResult.orderSummary.grandTotal, 115);
        ...
        const wallet = await WalletEngine_1.WalletEngine.getOrCreateWallet(customer._id);
        assert_1.default.strictEqual(wallet.availableBalance, 0);
        // Assert withdrawal with zero balance fails
        await assert_1.default.rejects(WalletEngine_1.WalletEngine.processDirectWithdrawal(customer._id, 100, {
            category: 'Delivery Withdrawal',
            source: 'BANK_TRANSFER',
            remarks: 'Should fail',
        }), /Insufficient wallet balance/);
        // Credit funds and verify direct withdrawal succeeds
        await WalletEngine_1.WalletEngine.credit(customer._id, 150, {
            category: 'Adjustment',
            source: 'manual',
            remarks: 'Add test funds',
        });
        const updatedWallet = await WalletEngine_1.WalletEngine.processDirectWithdrawal(customer._id, 100, {
            category: 'Delivery Withdrawal',
            source: 'BANK_TRANSFER',
            remarks: 'Should pass',
        });
        assert_1.default.strictEqual(updatedWallet.availableBalance, 50);
        assert_1.default.strictEqual(updatedWallet.withdrawnBalance, 100);
        // Verify WalletTransaction ledger created
        const tx = await WalletTransaction_1.WalletTransaction.findOne({ userId: customer._id, type: 'withdrawal' });
        assert_1.default.ok(tx);
        assert_1.default.strictEqual(tx.amount, 100);
        ...
        await inventoryService_1.InventoryService.reserveStock(order._id, customer._id, [{ productId: product._id.toString(), quantity: 1, variantId: null }]);
        // Test transition Placed -> Confirmed
        const confirmedOrder = await OrderStateMachine_1.OrderStateMachine.transition(order._id, 'Confirmed', {
            notes: 'Test confirmation'
        });
        assert_1.default.strictEqual(confirmedOrder.orderStatus, 'Confirmed');
        // Test illegal transition Placed -> Delivered
        // Confirmed -> Delivered is legal (since we allowed it), but Placed -> Delivered is illegal
        const placingOrder = new Order_1.Order({
        ...
        const cancelledOrder = new Order_1.Order({
        ...
        await assert_1.default.rejects(OrderStateMachine_1.OrderStateMachine.transition(cancelledOrder._id, 'Confirmed', { notes: 'Illegal jump' }), /Invalid order status transition/);
