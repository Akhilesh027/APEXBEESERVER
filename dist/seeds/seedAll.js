...
const main = async () => {
    ...
    if (isValidate) {
        ...
        try {
            ...
            for (const cat of cats) {
                if (!cat.seedKey)
                    throw new Error(`Category is missing "seedKey": ${JSON.stringify(cat)}`);
                if (!cat.name)
                    throw new Error(`Category is missing "name": ${JSON.stringify(cat)}`);
                if (!cat.slug)
                    throw new Error(`Category is missing "slug": ${JSON.stringify(cat)}`);
                if (categoryKeys.has(cat.seedKey))
                    throw new Error(`Duplicate Category seedKey: ${cat.seedKey}`);
            ...
            for (const sub of subs) {
                if (!sub.seedKey)
                    throw new Error(`Subcategory is missing "seedKey": ${JSON.stringify(sub)}`);
                if (!sub.categorySeedKey)
                    throw new Error(`Subcategory is missing "categorySeedKey": ${JSON.stringify(sub)}`);
                if (!sub.name)
                    throw new Error(`Subcategory is missing "name": ${JSON.stringify(sub)}`);
                if (!sub.slug)
                    throw new Error(`Subcategory is missing "slug": ${JSON.stringify(sub)}`);
                if (subcategoryKeys.has(sub.seedKey))
                    throw new Error(`Duplicate Subcategory seedKey: ${sub.seedKey}`);
                if (subcategorySlugs.has(sub.slug))
                    throw new Error(`Duplicate Subcategory slug: ${sub.slug}`);
            ...
            for (const prod of prods) {
                if (!prod.seedKey)
                    throw new Error(`Product is missing "seedKey": ${JSON.stringify(prod)}`);
                if (!prod.subcategorySeedKey)
                    throw new Error(`Product is missing "subcategorySeedKey": ${JSON.stringify(prod)}`);
                if (!prod.name)
                    throw new Error(`Product is missing "name": ${JSON.stringify(prod)}`);
                if (!prod.slug)
                    throw new Error(`Product is missing "slug": ${JSON.stringify(prod)}`);
                if (productKeys.has(prod.seedKey))
                    throw new Error(`Duplicate Product seedKey: ${prod.seedKey}`);
                if (productSlugs.has(prod.slug))
                    throw new Error(`Duplicate Product slug: ${prod.slug}`);
            ...
            for (const vr of vars) {
                if (!vr.seedKey)
                    throw new Error(`Variant is missing "seedKey": ${JSON.stringify(vr)}`);
                if (!vr.productSeedKey)
                    throw new Error(`Variant is missing "productSeedKey": ${JSON.stringify(vr)}`);
                if (!vr.sku)
                    throw new Error(`Variant is missing "sku": ${JSON.stringify(vr)}`);
                if (variantKeys.has(vr.seedKey))
                    throw new Error(`Duplicate Variant seedKey: ${vr.seedKey}`);
                if (variantSkus.has(vr.sku))
                    throw new Error(`Duplicate Variant SKU: ${vr.sku}`);
    ...
    try {
        const stats = await (0, seedProducts_1.seedCatalog)(isDryRun);
