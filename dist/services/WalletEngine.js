...
class WalletEngine {
     ...
     */
    static async runInTransaction(session, fn) {
        if (session) {
    ...
    static async credit(userId, amount, params, session) {
        return this.runInTransaction(session, async (sess) => {
            ...
            const result = await Wallet_1.Wallet.findOneAndUpdate({ userId }, {
                $inc: {
    ...
    static async hold(userId, amount, params, session) {
        return this.runInTransaction(session, async (sess) => {
            ...
            if (opKey) {
                const existingTx = await WalletTransaction_1.WalletTransaction.findOne({ operationKey: opKey }).session(sess);
                if (existingTx) {
    ...
    static async release(userId, amount, params, session) {
        return this.runInTransaction(session, async (sess) => {
            ...
            let result = await Wallet_1.Wallet.findOneAndUpdate({
                userId,
                ledgerEntries: {
                    $elemMatch: {
                        status: 'pending',
