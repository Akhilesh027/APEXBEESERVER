...
exports.getCheckoutQuote = void 0;
...
const getCheckoutQuote = async (req, res) => {
    try {
        const { items, lat, lng, couponCode, useWallet, deliverySlotId, } = req.body;
        ...
        for (const item of items) {
            const storeProduct = await StoreProduct_1.StoreProduct.findById(item.storeProductId).populate('productId');
            if (!storeProduct) {
            ...
            if (stockStatus !== 'in_stock') {
                return res.status(400).json({
                    success: false,
                    message: `Insufficient stock for product "${storeProduct.productId.name}". Available: ${inventory ? inventory.availableStock - inventory.reservedStock : 0}`,
                });
            }
        ...
        const store = await Vendor_1.Vendor.findById(primaryStoreId);
        if (!store) {
        ...
        if (couponCode) {
            const coupon = await Coupon_1.Coupon.findOne({ code: couponCode.toUpperCase(), isActive: true });
            if (coupon) {
